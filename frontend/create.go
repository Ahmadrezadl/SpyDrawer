...
func ssrCreateLobby(w http.ResponseWriter, r *http.Request) {
	...
	numberOfSpies, numberOfSpiesInvalid := api.ParseInt("number_of_spies", r.Form.Get("number_of_spies"), 0, 10)
	voteDuration, voteDurationInvalid := api.ParseInt("vote_duration", r.Form.Get("vote_duration"), 5, 60)
	...
	pageData := LobbyCreatePageData{
		...
		NumberOfSpies:     r.Form.Get("number_of_spies"),
		VoteDuration:      r.Form.Get("vote_duration"),
	}
	...
	if numberOfSpiesInvalid != nil {
		pageData.Errors = append(pageData.Errors, numberOfSpiesInvalid.Error())
	}
	if voteDurationInvalid != nil {
		pageData.Errors = append(pageData.Errors, voteDurationInvalid.Error())
	}
	...
	player, lobby, createError := game.CreateLobby(playerName, language, publicLobby, drawingTime, rounds, maxPlayers, customWordChance, clientsPerIPLimit, customWords, enableVotekick, wordsPerRound, numberOfSpies, voteDuration)
	if createError != nil {

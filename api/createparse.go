...
func ParseInt(valueName string, value string, min, max int) (int, error) {
	if value == "" {
		return min, nil
	}
	...
	result, parseErr := strconv.ParseInt(value, 10, 64)
	if parseErr != nil {
		return 0, fmt.Errorf("%s باید عدد باشد", valueName)
	}

	if int(result) < min {
		return 0, fmt.Errorf("%s نباید از %d کمتر باشد", valueName, min)
	}

	if int(result) > max {
		return 0, fmt.Errorf("%s نباید از %d بیشتر باشد", valueName, max)
	}

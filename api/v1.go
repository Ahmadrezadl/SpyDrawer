...
func createLobby(w http.ResponseWriter, r *http.Request) {
	...
	numberOfSpies, numberOfSpiesInvalid := ParseInt("number_of_spies", r.Form.Get("number_of_spies"), 0, 10)
	voteDuration, voteDurationInvalid := ParseInt("vote_duration", r.Form.Get("vote_duration"), 5, 60)
	...
	if numberOfSpiesInvalid != nil {
		requestErrors = append(requestErrors, numberOfSpiesInvalid.Error())
	}
	if voteDurationInvalid != nil {
		requestErrors = append(requestErrors, voteDurationInvalid.Error())
	}
...
func editLobby(w http.ResponseWriter, r *http.Request) {
	...
	lobby.Synchronized(func() {
		...
		lobby.NumberOfSpies = numberOfSpies
		lobby.VoteDuration = voteDuration
